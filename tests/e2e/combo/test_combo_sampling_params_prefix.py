...
def test_cache_hit_with_deterministic_sampling(tpu_llm: LLM):
    ...
    prompt1 = _apply_template(tpu_llm, SYSTEM_CONTEXT, "What is 5 + 5? Answer with just the number:")
    prompt2 = _apply_template(tpu_llm, SYSTEM_CONTEXT, "What is 5 + 5? Answer with just the number:")
...
def test_cache_hit_with_random_sampling(tpu_llm: LLM):
    ...
    prompt = _apply_template(tpu_llm, SYSTEM_CONTEXT, "Write a completely random and creative single word:")
    ...
    assert len(unique_outputs) > 1, "Random sampling failed to produce varied outputs on cached prompt."
...
def test_mixed_sampling_params_on_cached_prefix(tpu_llm: LLM):
    ...
    prompt_greedy = _apply_template(tpu_llm, SYSTEM_CONTEXT, "Task A: Reply with the exact word 'APPLE' and nothing else.")
    prompt_random = _apply_template(tpu_llm, SYSTEM_CONTEXT, "Task B: Write a highly creative and random poem line.")
...
def test_logprobs_with_prefix_caching(tpu_llm: LLM):
    ...
    prompt = _apply_template(tpu_llm, SYSTEM_CONTEXT, "Explain AI in three words:")
