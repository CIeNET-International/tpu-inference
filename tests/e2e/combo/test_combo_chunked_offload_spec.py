...
def _test_combo_chunked_offload_spec_accuracy(
    monkeypatch: pytest.MonkeyPatch,
    model_name: str,
    speculative_config: Optional[Union[dict, str]],
    enable_chunked_prefill: bool,
    async_scheduling: Union[bool, str] = "auto",
    cpu_chunks: str = "8",
    ...
    with monkeypatch.context():
        ...
        os.environ['TPU_OFFLOAD_BATCHED_SAVE'] = '0'
        ...
        if async_scheduling == "auto":
            async_scheduling_val = not speculative_config
        else:
            async_scheduling_val = bool(async_scheduling)
        ...
        llm = LLM(
            ...
            enable_chunked_prefill=enable_chunked_prefill,  # Chunked Prefill Switch
            kv_transfer_config=kv_transfer_config,          # F03: KV Cache Offloading
            speculative_config=speculative_config,          # F10: Speculative Decoding
        )
        ...
        for i in range(len(out_texts1)):
            assert out_texts1[i] == out_texts2[i], f"Text mismatch in request {i}"
            assert out_tokens1[i] == out_tokens2[i], f"Token mismatch in request {i}"
...
@pytest.mark.parametrize("async_scheduling", [
    pytest.param(True, marks=pytest.mark.xfail(reason="Async scheduling is not supported with speculative decoding on TPU backend yet", strict=False)),
    pytest.param(False)
])
def test_combo_chunked_ngram_llama_3b(monkeypatch: pytest.MonkeyPatch, enable_chunked_prefill: bool, async_scheduling: bool):
    """Tests Llama-3.2-3B-Instruct using Ngram Speculative Decoding, Chunked Prefill, and KV Offloading"""
    ...
    _test_combo_chunked_offload_spec_accuracy(
...
def test_combo_chunked_eagle3_llama_8b(monkeypatch: pytest.MonkeyPatch, enable_chunked_prefill: bool, async_scheduling: bool):
    """Tests Llama-3 8B using Eagle3 Speculative Decoding, Chunked Prefill, and KV Offloading"""
    model_name = os.environ.get("MODEL_NAME", "meta-llama/Meta-Llama-3.1-8B-Instruct")
    draft_model = os.environ.get("DRAFT_MODEL_NAME", "unkmaster/EAGLE3-LLaMA3.1-Instruct-8B")
