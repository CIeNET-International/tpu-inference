...
@pytest.mark.parametrize("async_scheduling", [
    pytest.param(
        ...
        marks=pytest.mark.skip(
            reason=
            "Async scheduling is not supported with ngram speculative decoding on vllm TPU backend."
        )),
    pytest.param(False)
])
def test_combo_chunked_ngram_llama_3b(monkeypatch: pytest.MonkeyPatch,
                                      enable_chunked_prefill: bool,
...
def test_combo_chunked_eagle3_llama_8b(monkeypatch: pytest.MonkeyPatch,
                                       enable_chunked_prefill: bool,
                                       async_scheduling: bool):
